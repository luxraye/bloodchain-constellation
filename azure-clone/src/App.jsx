...
function App() {
  ...
  return (
    <div className="flex bg-slate-100 min-h-screen w-full justify-center">
      <div className="flex min-h-screen flex-col bg-slate-50 text-slate-900 w-full max-w-5xl relative shadow-sm border-x border-slate-200">
        <Navbar profileName={profile?.name} />
        <main className="flex-1 overflow-y-auto overflow-x-hidden p-6 pb-24 relative">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === TABS.PROFILE && <Profile session={user} profile={profile} onProfileUpdate={setProfile} />}
            </motion.div>
          </AnimatePresence>
        </main>
...
function LoginScreen() {
  ...
  const [form, setForm] = useState({ email: '', password: '', name: '', bloodType: 'O+', confirm: '' })
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      const { error: authError } = await login(form.email.trim(), form.password)
  ...
  const handleRegister = async (e) => {
    ...
    try {
      await registerDonor({ email: form.email.trim(), password: form.password, name: form.name.trim(), bloodType: form.bloodType })
      // registerDonor auto-signs in via supabase after creating the account — no further action needed
    } catch (err) { setError(err.message || 'Registration failed') }
    finally { setLoading(false) }
  }

  return (
    <div className="flex bg-slate-200 min-h-screen w-full items-center justify-center p-4">
      <div className="flex bg-slate-50 flex-col px-8 py-10 w-full max-w-lg relative shadow-2xl rounded-2xl border border-slate-300 bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200">
        ...
        <div className="relative z-10 w-full mb-8 space-y-2">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 drop-shadow-sm">{mode === 'signin' ? 'Welcome back' : 'Create account'}</h1>
          <p className="text-sm font-medium text-slate-500">{mode === 'signin' ? 'Track your donations and emergency blood requests.' : 'Join the Bloodchain network as a registered donor.'}</p>
        </div>

        {/* Toggle with glassmorphism */}
        <div className="relative z-10 mb-6 flex rounded-xl border border-white/40 bg-white/30 p-1 backdrop-blur-md shadow-inner">
          {['signin', 'register'].map(m => (
            <button key={m} onClick={() => { setMode(m); setError('') }}
              className={`flex-1 rounded-lg py-2.5 text-sm font-semibold transition-all duration-300 ${mode === m ? 'bg-white text-slate-900 shadow-sm scale-100' : 'text-slate-500 hover:text-slate-700 scale-95'
                }`}>
            ...
            <form onSubmit={handleSignIn} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input id="password" type={showPw ? 'text' : 'password'} autoComplete="current-password" value={form.password} onChange={set('password')} placeholder="••••••••" required
                    className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-10 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-red-500" />
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input type="text" value={form.name} onChange={set('name')} placeholder="Kagiso Sithole" required
                    className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-red-500" />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-700">Email address</label>
