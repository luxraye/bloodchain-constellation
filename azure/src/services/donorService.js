...
const DEMO_CENTERS = [
    { id: 'ctr-001', name: 'NBTS Gaborone', county: 'Gaborone', distanceKm: 4.2, etaMinutes: 12, status: 'OPEN' },
    { id: 'ctr-002', name: 'Princess Marina Hospital', county: 'Gaborone', distanceKm: 7.4, etaMinutes: 18, status: 'OPEN' },
]

const DEMO_SLOTS = [
    { id: 'slot-001', date: new Date(Date.now() + 1000 * 60 * 60 * 24 * 2).toISOString(), window: '09:00 - 09:30', centerName: 'NBTS Gaborone', capacityRemaining: 3, bookedAt: null },
...
export async function getDonationHistory() {
    if (isGuestDemo()) return readGuestState().donations ?? DEMO_DONATIONS
...
export async function getDonationJourney() {
    if (isGuestDemo()) {
        return [
            { id: 'j-1', label: 'Donation captured', description: 'Your unit was registered at NBTS Gaborone.', date: new Date(Date.now() - 1000 * 60 * 60 * 24 * 88).toISOString(), status: 'DEFAULT', meta: { location: 'NBTS Gaborone' } },
...
export async function getAssetCustody(assetId) {
    if (isGuestDemo()) return { id: assetId, events: [] }
...
export async function getNationalStockLevels() {
    if (isGuestDemo()) return DEMO_STOCK
    const { data } = await apiClient.get('/admin/stats')
    return data?.data ?? data ?? null
}
export async function getDonationCenters() { if (isGuestDemo()) return DEMO_CENTERS; throw new Error('Donation centers endpoint is not implemented yet') }
export async function getScheduleSlots() { if (isGuestDemo()) return readGuestState().slots ?? DEMO_SLOTS; throw new Error('Schedule slots endpoint is not implemented yet') }
...
export async function getDonationEducation() { if (isGuestDemo()) return DEMO_EDUCATION; throw new Error('Donation education endpoint is not implemented yet') }
export async function getNearbyRequests() { if (isGuestDemo()) return readGuestState().nearbyRequests ?? DEMO_NEARBY_REQUESTS; throw new Error('Nearby requests endpoint is not implemented yet') }
...
export async function getRequests() { if (isGuestDemo()) return readGuestState().requests ?? DEMO_REQUESTS; throw new Error('Requests endpoint is not implemented yet') }
